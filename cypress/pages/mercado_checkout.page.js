        ...
        cy.readFile('cypress/fixtures/cadastro_endereco/endereco.json').then(endereco =>{ 
            cy.get('.ch-modal-content').should('not.be.visible')
                ...
                cy.get("[data-tray-tst=security_question]").then($securityQuestion => {
                    if ($securityQuestion.find("p:contains('Qual seu endereço de entrega?')").length > 0){
                        selecionar = endereco.valido.rua.split(' ')[1]
                    } else if ($securityQuestion.find("p:contains('Quais os primeiros digitos do seu CPF?')").length > 0) {
